...
typedef struct __attribute__((packed)) _RIP_MSG_FIXED
{
  ...
  void Dhcp_send_DHCP_MESSAGE(SOCKET sock, uint8_t, uint16_t);
  void Dhcp_printByte(char * buf, uint8_t n);

  uint8_t Dhcp_parseDHCPResponse(SOCKET sock, unsigned long responseTimeout, uint32_t* transactionId);
  ...
  int Dhcp_beginWithDHCP(SOCKET sock, uint8_t *mac, unsigned long timeout, unsigned long responseTimeout);
