...
bool RFPacket_check_crc16(uint8_t *buffer) {
	...
	uint8_t len = 10 + buffer[7];														// 10 + ...
	if (len < 255) {
		uint16_t chksum_received = (((uint16_t) buffer[8]) << 8 | buffer[9]);			// [8] und [9] bei alt
		buffer[8] = 0x00;
		buffer[9] = 0x00;
...
bool RFPacket_check_crc16_DWM1000(uint8_t *buffer) {

	char string_buffer[256] = { 0 }; // UART String Buffer
	bool checksum_ok = false;
	uint8_t len = 12 + buffer[7];														// 10 + ...
	if (len < 255) {
		uint16_t chksum_received = (((uint16_t) buffer[10]) << 8 | buffer[11]);			// [8] und [9] bei alt
		buffer[10] = 0x00;
		buffer[11] = 0x00;
		uint16_t chksum_computed = CalcBlockCrc(buffer, len);
		...
		buffer[10] = chksum_received >> 8;
		buffer[11] = chksum_received;
	}
...
uint8_t RFPacket_check_and_remove_header_DWM1000(uint8_t *data, uint32_t *SOURCE, uint32_t *DEST, uint8_t *MSG_TYPE) {
	// Read source address
	uint32_t source_address = (((uint32_t) data[0]) << 16	| ((uint32_t) data[1]) << 8 | data[2]);
	if (*SOURCE == 0x00) {  // If Source is empty, return source address
		*SOURCE = source_address;
	...
	// Check CRC
	if (!RFPacket_check_crc16_DWM1000(data)) {
		return 0;
	}
