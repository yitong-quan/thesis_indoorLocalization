...
%     %file_name = '25ms_40HzSamplingRate\stateVector.mat'; 1state_space1_good
%     file_name = '25ms_40HzSamplingRate\1state_space1_good.mat'; 
%     real_X = importdata(file_name); 
%     %real_X = real_X(:,1:100*5);%(:,1:end);
%     ms25 = 0;
%     
% %{
...
%             b = [1:5:size(real_X,2)];
%         real_X= real_X(:,b);
%         temp_loop = zeros(size(distances2all_abs,1),size(real_X,2));
%         for iijj = 1:size(real_X,2) %100
%             temp_dist = distances2all_abs(:,(iijj*5-4):(iijj*5));
%             for jjii = 1:size(distances2all_abs,1)
%                 temp_loop(jjii, iijj) = temp_dist(jjii,jjii);
%             end
%         end
%         distances2all_abs = temp_loop;
% end    
% %}
...
%     
%             sigma = 0.5; %sigma should be the same as the title of the traj_plot
% 
...
%     str = sprintf('%0.20f RMSD, R %0.3f, Q %0.3f, sigma %0.3f', RMSD_, factor_R, factor_Q, sigma);
%     title(str);
%     str = [str, '   .fig'];
...
%% nodes position 
    ...
    %    1  3  ]
%%
function [X, P, z_all, RMSD_] = EKF_25ms40Hz(factor_Q, factor_R, experimentNumber) %, measurements_missing, MaxNumMeasMissedWithinSet, traj_name
    format longG
    ...
    switch experimentNumber
        case 2 % simulation
            real_x_t = importdata('25ms_40HzSamplingRate\2_stateVector.mat');
        case 5 % simulation
        ...
        otherwise
            ...
            if ms25 == 1                 
                temp_loop = NaN(size(distances2all_abs,1),size(real_X,2));
                for iijj = 1:size(real_X,2) %100*5
                    temp_dist_i = NaN(nodes_Nums,1);
                    row_index = (iijj-floor(iijj/nodes_Nums)*nodes_Nums);
                    if row_index == 0
                       row_index=5;
                    end
                    temp_dist_i(row_index)= distances2all_abs(row_index,iijj);
                    temp_loop(:,iijj)=temp_dist_i;
                end
                distances2all_abs = temp_loop;
            else 
                b = [1:5:size(real_X,2)];
    ...
    data = [t',measurements_data_noisy];
...
%     time_last = timeStamp(1);
    ...
    for i = 2:size(measurements_data_noisy,2)
        ...
        dt = timeStamp(i) - timeStamp(i-counterJumpMeasLess);
...
%         dt = time_now - time_last;
        ...
        if mitigation_linear_comb == 1
            ...
            elseif z_length <= 3
                ...
                residual_sum_array = zeros(1,numCombinations);
...
%     end
    ...
    % ----------- close(video)
    hh=figure;
    subplot(3,3,1);histogram(mis_pos(1,:));
    subplot(3,3,2);histfit(mis_pos(1,:));
    subplot(3,3,3);histfit(mis_pos(1,:),10,'rayleigh');title('mis pose x');
    
    subplot(3,3,4);histogram(mis_pos(2,:));
    subplot(3,3,5);histfit(mis_pos(2,:));
    subplot(3,3,6);histfit(mis_pos(2,:),15,'rayleigh');title('mis pose y');
    
    subplot(3,3,7);histogram(mis_dist);
    subplot(3,3,8);histfit(mis_dist);
    subplot(3,3,9);histfit(mis_dist,17,'rayleigh');title('mis dist ');
    %% compare the velocities

mis_v_x = X(3,:) - real_X(3,:);
mis_v_y = X(4,:) - real_X(4,:);
mis_v_sum = sqrt(mis_v_x.^2 + mis_v_y.^2);
       hhh = figure;
    subplot(3,3,1);histogram(mis_v_x);
    subplot(3,3,2);histfit(mis_v_x);
    subplot(3,3,3);histfit(mis_v_x,12,'rayleigh');title('mis vel x');
    
    subplot(3,3,4);histogram(mis_v_y);
    subplot(3,3,5);histfit(mis_v_y);
    subplot(3,3,6);histfit(mis_v_y,12,'rayleigh');title('mis vel y');
    
    subplot(3,3,7);histogram(mis_v_sum);
    subplot(3,3,8);histfit(mis_v_sum);
    subplot(3,3,9);histfit(mis_v_sum,12,'rayleigh');title('mis vel sum ');
    ...
    test_normal(2,6) = jbtest(mis_v_sum);
            test_normal(3,1) = kstest(mis_pos(1,:));
    test_normal(3,2) = kstest(mis_pos(2,:));
    test_normal(3,3) = kstest(mis_dist);
    test_normal(3,4) = kstest(mis_v_x);
    test_normal(3,5) = kstest(mis_v_y);
    test_normal(3,6) = kstest(mis_v_sum);
                test_normal(4,1) = chi2gof(mis_pos(1,:));
    test_normal(4,2) = chi2gof(mis_pos(2,:));
    test_normal(4,3) = chi2gof(mis_dist);
    test_normal(4,4) = chi2gof(mis_v_x);
    test_normal(4,5) = chi2gof(mis_v_y);
    test_normal(4,6) = chi2gof(mis_v_sum);
