...
bool AS3933_receive_data(uint32_t *woke_up_by_ID, uint8_t * rssi, uint8_t * options, uint32_t SPI_BAUDRATE) {
	uint16_t numByte = 0;
	uint16_t numBit = 0;
	uint8_t length = 1;
	...
	while (numByte <= length) {
		...
		if (numBit == 7) {
			...
			numByte++;
			if (numByte == 1) {
				*rssi = AS3933_spi_read_reg(AS3933_REG10) & 0x1F;
	...
	if (WAKEUP_24BIT) {
		// Check CRC
		chksum_computed = CalcBlockCrc(tmp, 5);
		chksum_received = (((uint16_t) received_payload[6]) << 8 | received_payload[7]);
		...
		*options = received_payload[5];
		...
		chksum_computed = CalcBlockCrc(tmp, 4);
		chksum_received = (((uint16_t) received_payload[5]) << 8 | received_payload[6]);
		if (chksum_computed != chksum_received) {
			return false;
		}
		*woke_up_by_ID = (((uint32_t) received_payload[1]) << 16 | ((uint32_t) received_payload[2]) << 8 | received_payload[3]);
		*options = received_payload[4];
