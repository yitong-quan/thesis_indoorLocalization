...
*/
                ...
                // Receive Node IDs
/*
                        // FLAG5: DO NOTHING FOR 0,15 SECOND; FOR OBSERVATION IN THE Oscilloscope
                                RTC_delay_ms(100);
                                for(int i = 0; i < 10; i++){
                                        LED_setLED(COL_RED);
                                        RTC_delay_ms(2);
...
*/
//		RTC_start(200);													// 1200 //2000
                //below are what is inside RTC_star(), with a little change of the codes order
                uint16_t timeout_ms = 200;
                RTC_Enable(true);  // Start RTC (for timeout)  //TODOYitong can be deleted
                RTC_CounterReset();
                RTC_CompareSet(0, (((32768 * timeout_ms) / 1000) - 1));
                RTC_TIMEOUT = false;
                //above are what is inside RTC_star(), with a little change of the codes order
...
/*
                        // FLAG10: DO NOTHING FOR 0,018 SECOND; FOR OBSERVATION IN THE Oscilloscope
                        for(int i = 0; i < 6; i++){
                                LED_setLED(COL_RED);
                                RTC_delay_ms(1);
                                LED_clearLED();
...
*/
                        if (!RFPacket_check_and_remove_header(CC1101_receive, &ID_WHO_WOKE_ME_UP, &MY_TAG_ID, &MSG_TYPE))
                        {
                                ...
                                /*
                        		// FLAG9: DO NOTHING FOR 0,1 SECOND; FOR OBSERVATION IN THE Oscilloscope
...
/*
                        // FLAG8: DO NOTHING FOR 0,040 SECOND; FOR OBSERVATION IN THE Oscilloscope
                        for(int i = 0; i < 10; i++){
                                LED_setLED(COL_RED);
                                RTC_delay_ms(2);
                                LED_clearLED();
...
*/
                        ...
                        cc1101_ack_reception(&xfer, &MY_TAG_ID, &ID_WHO_WOKE_ME_UP, AES_encryption_key, AES_initVector);
                        /*
                        // FLAG12 for _ack_: DO NOTHING FOR 0,06 SECOND; FOR OBSERVATION IN THE Oscilloscope
                                 for(int i = 0; i < 6; i++){
                                         LED_setLED(COL_RED);
                                         RTC_delay_ms(5);
                                         LED_clearLED();
                        ...
                        */
/*
                        // FLAG7: DO NOTHING FOR 0,064 SECOND; FOR OBSERVATION IN THE Oscilloscope
                        for(int i = 0; i < 16; i++){
                                LED_setLED(COL_RED);
                                RTC_delay_ms(2);
                                LED_clearLED();
...
*/
                        //TODO below To be checked in the Oscilloscope  !!!!!!!!!! Yitong !!!!!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&!!!&&&&&&
                        //RTC_CounterReset();
                        //RTC_TIMEOUT = true;
                        break;
                        //TODO above To be checked in the Oscilloscope  !!!!!!!!!! Yitong !!!!!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&!!!&&&&&&
                }
/*
                // FLAG4: DO NOTHING FOR 0,15 SECOND; FOR OBSERVATION IN THE Oscilloscope
                        RTC_delay_ms(100);
                        for(int i = 0; i < 5; i++){
                                LED_setLED(COL_RED);
                                RTC_delay_ms(4);
...
*/
                ...
                DWM1000_Chip_INIT();
/*
                        // FLAG1: DO NOTHING FOR 0,15 SECOND; FOR OBSERVATION IN THE Oscilloscope
                                RTC_delay_ms(100);
                                for(int i = 1; i < 5; i++){
                                        LED_setLED(COL_RED);
                                        RTC_delay_ms(5);
