...
extern uint8_t BLINK_message[];								// Blink message
	...
	while(measurement_numbers > 0)
	{
		switch (state1)
		{
			case STATE_POLL_TRANSMIT:
				state1 = DWM1000_poll_transmit_state(ID_WHOWOKE_ME_UP, MY_NODE_ID, &poll_tx_timestamp);
				RTC_start(80);
			break;
			...
			case STATE_RESPONSE_RECEIVE:
				...
				state1 = DWM1000_response_receive_state(rx_buffer, &ID_WHOWOKE_ME_UP, &MY_NODE_ID, &MSG_TYPE, &resp_rx_timestamp);
			break;

			case STATE_FINAL_TRANSMIT:
				DWM1000_send_final_message(FINAL_message, ID_WHOWOKE_ME_UP, MY_NODE_ID, FINAL_MESSAGE_LENGTH, poll_tx_timestamp, resp_rx_timestamp);
				measurement_numbers--;
				state1 = STATE_POLL_TRANSMIT;
				RTC_delay_ms(10);
			break;
	...
	while(measurement_numbers && count > 0)
	{
		switch (state)
		{
			case STATE_SEND_BLINK:
				...
				RTC_start(10);
				//>>>>>>>>>>>>>>>>>>>>>>>>> Inorder to make the time needed shorter, when not all th enodes can be contacted. can it be Changed to 300?? TODO Yitong
...
{
	DWM1000_receiver();
	DWM1000_send_blink_message(BLINK_message, DESTIN_ID, SOURCE_ID, BLINK_MESSAGE_LENGTH);			// send blink message to NODES immediate

	return  STATE_RECEIVE_MODE;																		// go to Receive Mode to get the Poll Message from the Nodes
}
