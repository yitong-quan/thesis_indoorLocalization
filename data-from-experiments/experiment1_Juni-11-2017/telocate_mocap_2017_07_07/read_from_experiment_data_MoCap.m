...
%{
for iii = 1: size(Time_Position,1)
    hold on; plot(Time_Position(iii,2), Time_Position(iii,3), '.');
    disp(iii);
    pause(0.00000001);
end
%}
figure; plot(Time_Position(:,2), '.');
hold on; plot(Time_Position(:,3), '.');
hold on; plot(Time_Position(:,4), '.');
